...
// #creative-affinity-feedback
const SLACK_CHANNEL_ID = process.env.SLACK_CHANNEL_ID || "C0ANW7CFQUT";
...
async function fetchIntercom() {
  ...
  do {
    ...
    for (const c of (json.conversations || [])) {
      ...
      const cr = c.conversation_rating || {};
      const ai = c.ai_agent || {};
      out.push({
        id: String(c.id),
        ...
        rating: (cr.rating ?? null),                  // CSAT 1-5, null if unrated
        ratingRemark: (cr.remark ?? null),
        aiParticipated: !!(c.ai_agent_participated ?? (ai && Object.keys(ai).length>0)),
        aiResolution: (ai.resolution_state ?? null),  // assumed_resolution | confirmed_resolution | escalated | ...
...
// ---------------------------------------------------------------------------
// Slack — #creative-affinity-feedback, classified into Bug / Feature / User error
// ---------------------------------------------------------------------------
// Keyword rules. A message is only counted if it clearly matches ONE bucket.
// Ambiguous messages (questions, announcements, "which expert can join a call")
// are intentionally left uncounted rather than force-fit.
function classifyFeedback(text) {
  const s = " " + (text || "").toLowerCase().replace(/\s+/g, " ") + " ";

  // Feature request — asks for new capability / availability
  const feature = [
    "would it be possible", "can we get", "could we get", "is it on the roadmap",
    "on the roadmap", "feature request", "add a", "ability to", "would love to get",
    "would love a", "when are we expecting", "when will", "eta on", "is there a way to",
    "can you add", "please add", "integrate", "integration of", "roll out", "rolled out"
  ];
  // Bug — something broken / wrong / erroring
  const bug = [
    "not working", "isn't working", "doesn't work", "not pulling", "not showing",
    "not updating", "error", "broken", "bug", "incorrect", "incorrectly", "wrong",
    "inaccurate", "not functioning", "not lining up", "don't match", "doesn't match",
    "not match", "reverting", "duplicates", "no active ads", "no ads", "not accurate",
    "keeps reverting", "issue:", "failing", "fails", "can't save", "cannot save"
  ];
  // User error — the person did something wrong / needs how-to, not a product defect
  const userErr = [
    "how do i", "how do you", "am i doing", "did i do", "i can't find", "cant find",
    "where do i", "where is the", "nvm", "never mind", "my mistake", "user error",
    "figured it out", "i was doing", "wrong link", "used the wrong", "operator error",
    "disregard", "my bad"
  ];

  // Strong user-error signals: self-resolved mistakes. These win outright,
  // because the person is telling us it was their own error, not a defect.
  const strongUserErr = [
    "nvm", "never mind", "my mistake", "my bad", "figured it out",
    "user error", "operator error", "disregard", "used the wrong", "wrong link"
  ];

  const hit = (arr) => arr.some(k => s.includes(k));
  const isFeature = hit(feature);
  const isBug = hit(bug);
  const isUserErr = hit(userErr);

  // Priority: strong self-resolved language wins first, then remaining
  // user-error how-to phrases, then bug, then feature. Only count clear cases.
  if (hit(strongUserErr)) return "userError";
  if (isUserErr && !isBug) return "userError";
  if (isBug && !isFeature) return "bug";
  if (isFeature && !isBug) return "feature";
  if (isBug && isFeature) return null;   // mixed signal -> leave uncounted
  return null;                            // no clear signal -> uncounted
}

async function fetchSlack() {
  if (!SLACK_TOKEN) throw new Error("SLACK_TOKEN is not set");
  const out = { bug: [], feature: [], userError: [], total: 0, counted: 0 };
  let cursor = null;
  const YEAR_START_TS = "1767225600.000000"; // 2026-01-01

  do {
    const params = new URLSearchParams({
      channel: SLACK_CHANNEL_ID,
      limit: "200",
      oldest: YEAR_START_TS
    });
    ...
    for (const m of (json.messages || [])) {
      // only real user messages (skip joins, bot/system subtypes)
      if (m.subtype) continue;
      if (!m.text) continue;
      out.total++;
      const bucket = classifyFeedback(m.text);
      if (!bucket) continue;
      out.counted++;
      const tsId = (m.ts || "").replace(".", "");
      out[bucket].push({
        ts: m.ts,
        created: Math.floor(Number(m.ts) || 0),   // unix seconds
        text: m.text.slice(0, 280),
        replies: m.reply_count || 0,
        url: `https://power-digital.slack.com/archives/${SLACK_CHANNEL_ID}/p${tsId}`
      });
    }
    cursor = json.response_metadata && json.response_metadata.next_cursor
      ? json.response_metadata.next_cursor : null;
  } while (cursor);

  // newest first
  for (const k of ["bug", "feature", "userError"]) {
...
app.get("/api/data", async (_req, res) => {
  try {
    // Slack is fetched but must not break the whole dashboard if it fails
    // (e.g. token missing) — Intercom + Jira stay authoritative.
    const [intercom, jira, slackResult] = await Promise.all([
      fetchIntercom(),
      fetchJira(),
      fetchSlack().catch(err => { console.error("Slack fetch failed:", err.message); return null; })
    ]);
    const slack = slackResult;
    ...
    res.json({
      ...
      jira,
      slack,
      slackChannel: "creative-affinity-feedback",
